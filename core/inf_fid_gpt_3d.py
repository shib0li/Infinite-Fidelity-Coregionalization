...
class InfFidNet3D(nn.Module):
    ...
    def __init__(self, 
        ...
        super(InfFidNet3D, self).__init__()
        
        self.ns_t_dim = dataset.ns_t_dim
        ...
        self.KerT = KernelRBF()
        self.A = HoA(
            d=self.ns_t_dim*self.s_dim*self.s_dim, 
            K=self.h_dim, 
    ...
    def _eval_elbo_llh_t(self, ht, yt, tt):
        ...
        #================ For NS interp to 3D tensor ================#
        yt_3d = yt.reshape(
            [N, self.ns_t_dim, fid_t, fid_t])

        flat_tN_yt_3d =  yt_3d.reshape(
            [N*self.ns_t_dim, fid_t, fid_t])

        interp_flat_tN_y_3d = torch.nn.functional.interpolate(
            flat_tN_yt_3d.unsqueeze(1), 
            size=self.s_dim, 
            mode=self.interp,
        ).squeeze(1)

        interp_y_3d = interp_flat_tN_y_3d.reshape(
            [N, self.ns_t_dim, self.s_dim, self.s_dim])

        #============================= End ========================#
        
        re_yt = interp_y_3d.reshape([N, -1])
    ...
    def predict(self, X_list, t_list, t_list_tr):
        ...
        with torch.no_grad():
            ...
            for tid, tt in enumerate(t_list):
                ...
                #================ For NS interp to 3D tensor ================#
                pred_t = torch.mm(ht, At.T)
                pred_yt_3d = pred_t.reshape(
                    [Nt, self.ns_t_dim, self.s_dim, self.s_dim])

                flat_tN_pred_3d =  pred_yt_3d.reshape(
                    [Nt*self.ns_t_dim, self.s_dim, self.s_dim])

                interp_flat_tN_pred_3d = torch.nn.functional.interpolate(
                    flat_tN_pred_3d.unsqueeze(1), 
                    size=fid_t, 
                    mode=self.interp,
                ).squeeze(1)

                interp_pred_3d = interp_flat_tN_pred_3d.reshape(
                    [Nt, self.ns_t_dim, fid_t, fid_t])


                interp_pred_yt_1d = interp_pred_3d.reshape([Nt, -1])
    ...
    def eval_pred(self, X_list, t_list, t_list_tr):
        ...
        for tid, tt in enumerate(t_list):
            ...
            Nt = pred_t.shape[0]
            re_pred_t = pred_t.reshape([Nt, self.ns_t_dim, fid_t, fid_t])
