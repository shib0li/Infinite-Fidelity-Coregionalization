%% ProjetMOR_Burger1D_FEM_Part2_TestMOR_Mod2 
%% The script test all cases desinated rahter than one single case
%
% Modifications:
% 27-Aug-2015, WeiX, first edition 
...
new_dim=7;     % For LPCA more than 10 require. The new dimension of reduced emulatior
%----------------Load dataset-------------------------------------------
load('ExpData4.mat') %7 is ok for HH,
Num_Trian=40;   % 80 is best for 'ExpData3,4.mat'
Num_test=40;  

% Index_test_start=500-Num_test+1;  %500 should change accroding to the dataset

% X_star=[10,-10,5];
X_star=X(end-Num_test+1:end,:);
Y_orig=Y(end-Num_test+1:end,:);
...
Y_global_snaps=[];
for i=1:Num_Trian
   Y_train_snaps_i=reshape(Y(i,:)',Paras.n+1,[]);    
   Y_global_snaps=[Y_global_snaps,Y_train_snaps_i(2:end-1,:)];
end
[U_global,S,~]=svd(Y_global_snaps);  % U*S*V'=Rec_X
U_global=U_global(:,1:n_Ubases);
...
for i = 1: Num_test
    ...
    %% -----------------Local GPE basis ------------------------------------
    %-------------------------------Rearrange---------------------------------
    Y_star_snaps=reshape(Y_star(i,:)',Paras.n+1,[]);    
    %-------------------MOR FEM------------------------------------
    ...
    SE=(Y_U_star-Y_Full).^2;
    SSE_Y_U_star(i,1)=sum(SE(:));
    
    
    %% -----------------Global basis ------------------------------------
    [Y_U_global,~,Time_U_star]=Burger1D_FEM_DBC_MOR_SolverF(Paras,U_global);
    
    SE=(Y_U_global-Y_Full).^2;
    SSE_Y_U_global(i,1)=sum(SE(:));
...
figure(1)
boxplot([SSE_Y_U_global(:),SSE_Y_U_star(:)],'labels',{'Global Basis MOR FEM','GPE Predicted Basis MOR FEM'})
title(sprintf('L^2 Error on each test case'))
